<template>
    <svg id="svg1" width="100%" height="100%" style="border:1px solid black; background-color:white" viewBox="0 0 1000 700" class="container-border col-lg-12">
        <defs>
    ...
    methods: {
        ...
        init(){
            ...
            resetButton.append('path')
                .attr('transform', 'scale(0.85) translate(80, 153.6)')
            ...
            axios.get(process.env.VUE_APP_SERVER_URL + endpoint, { params })
                .then(response => {
                    ...
                    for (let t in neighbours) {
                        ...
                        for (let i in this.graphData.nodes) {
                            if (this.graphData.nodes[i].id === neighbID) {
                                node_is_new[t] = false;
                                num_new--;
                            }
                        }
